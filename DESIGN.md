...
├── cache.go               # Transparent read-caching built on eko/gocache
├── cache_invalidator.go   # Optional MongoDB change stream monitoring
...
Read operations (`Find`, `FindOne`, `CountDocuments`, `Aggregate`) can be transparently cached via `CachedCollection[T]`.

- **Deterministic Keys** — Keys are generated by hashing the collection namespace, query filter (stable ExtJSON), and query options (sort, limit, projection).
- **Collection-Wide Tagging** — Every cache entry for a collection is tagged with its namespace (e.g. `mydb.users`), enabling atomic invalidation for the entire collection.
- **Pluggable Backends** — Leverages `eko/gocache`, allowing the same API to use in-memory stores, Redis, or multi-level caching strategies.
- **Write-Through Invalidation Policy** — Write operations on `CachedCollection` always bypass the cache. Invalidation is either TTL-based or event-driven via change streams.

### Change Stream Invalidation

For replica-set or Atlas deployments, `ChangeStreamInvalidator` provides near real-time cache consistency:

1. **Watch Loop** — Monitors collection writes in the background.
2. **Atomic Invalidation** — Upon any write event, it flushes the collection-specific tag in the cache.
3. **Internalized State** — To remain "stateless," the invalidator persists its **resume token** directly in the cache backend under a reserved key.
4. **Resiliency** — Automatic reconnection with exponential backoff ensures monitoring resumes after transient network or MongoDB failures.
...
| `CachedCollection[T]` | Caching CRUD wrapper | Decorator over `Collection[T]`. |
| `ChangeStreamInvalidator` | Cache invalidator | Background watcher for MongoDB change streams. |
